...
func TestReElection2A(t *testing.T) {
	...
	log.Print("<<< Test(2A) stage1 passing >>>")

	// if the leader disconnects, a new one should be elected.
	cfg.disconnect(leader1)
	cfg.checkOneLeader()
	log.Print("<<< Test(2A) stage2 passing >>>")

	// if the old leader rejoins, that shouldn't
	// disturb the new leader.
	cfg.connect(leader1)
	leader2 := cfg.checkOneLeader()
	log.Print("<<< Test(2A) stage3 passing >>>")
	...
	time.Sleep(2 * RaftElectionTimeout)
	cfg.checkNoLeader()
	log.Print("<<< Test(2A) stage4 passing >>>")

	// if a quorum arises, it should elect a leader.
	cfg.connect((leader2 + 1) % servers)
	cfg.checkOneLeader()
	log.Print("<<< Test(2A) stage5 passing >>>")

	// re-join of last node shouldn't prevent leader from existing.
	cfg.connect(leader2)
	cfg.checkOneLeader()
	log.Print("<<< Test(2A) stage6 passing >>>")
...
func TestBackup2B(t *testing.T) {
	...
	leader1 := cfg.checkOneLeader()
	TestPrintf("2B", "Leader = %v", leader1)
